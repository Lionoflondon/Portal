...
function Events({ user, eventState, onFollow, following, onCreate }) {
  ...
  return <div className="page events-page events-canvas"><div className="events-atmosphere" aria-hidden="true" /><div className="welcome-head events-heading"><div><h1 className="display-xl">What is happening?</h1><p className="body-md">Portal Events is the world&apos;s timeline: incidents, culture, sport, weather, government, community and everything unfolding around us.</p></div><button type="button" className="btn btn-primary" onClick={onCreate}>Create event</button></div><div className="event-discovery-controls"><div className="glass event-filter-panel event-filter-primary"><div className="event-filter-group event-filter-status"><span className="eyebrow">Status</span><div className="chip-row event-filter-scroll">{['Nearby', 'Live', 'Breaking', 'Today', 'Upcoming', 'Following', 'Trending', 'Archived'].map((item) => <button type="button" className={`chip ${filter === item ? 'active' : ''}`} onClick={() => setFilter(item)} key={item}>{item}</button>)}</div></div><div className="event-filter-group event-filter-region"><span className="eyebrow">Region</span><div className="chip-row event-filter-scroll">{eventRegions.map((item) => <button type="button" className={`chip ${region === item ? 'active' : ''}`} onClick={() => setRegion(item)} key={item}>{item}</button>)}</div></div></div><div className="glass event-filter-panel event-filter-sector"><span className="eyebrow">Sector</span><div className="chip-row event-filter-scroll">{eventCategories.map((item) => <button type="button" className={`chip ${category === item ? 'active' : ''}`} onClick={() => setCategory(item)} key={item}>{item}</button>)}</div><span className="event-filter-chevron" aria-hidden="true">›</span></div></div><Section title="Happening now"><EventCollection events={filtered} loading={eventState.loading} error={eventState.error ? EVENTS_UNAVAILABLE_MESSAGE : ''} empty="No live events available." emptyDetail="Events will appear as trusted sources become available." onRefresh={() => window.location.reload()} onFollow={onFollow} following={following} /></Section><p className="body-sm events-session">Signed in as {user.email}</p></div>;
}
