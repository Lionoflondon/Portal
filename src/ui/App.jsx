...
function CreateModal({ open, onClose, user, events }) { const [busy, setBusy] = useState(false); const [error, setError] = useState(''); if (!open) return null; async function submit(values) { if (busy) return; setBusy(true); try { const record = await createPortalEvent(user, values); onClose(); window.location.hash = `#/events/${record.id}`; } catch (reason) { setError(firebaseMessage(reason)); } finally { setBusy(false); } } return <div className="modal-overlay" role="dialog" aria-modal="true" aria-labelledby="createModalTitle" onMouseDown={onClose}><div className="modal form-modal" onMouseDown={(event) => event.stopPropagation()}><div className="modal-head"><h2 id="createModalTitle">Create event</h2><button className="modal-close" type="button" onClick={onClose} aria-label="Close">×</button></div>{error ? <p className="form-error" role="alert">{error}</p> : null}<EventForm events={events} onSubmit={submit} onCancel={onClose} busy={busy} /></div></div>; }
