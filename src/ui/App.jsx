...
function Topbar({ profile, unreadCount = 0 }) { return <header className="topbar mobile-only"><Brand /><div className="topbar-actions"><a className="topbar-notification-link" aria-label={unreadCount ? `${unreadCount} unread notifications` : 'Open notifications'} href="#/notifications"><Icon name="notifications" />{unreadCount > 0 ? <span className="notification-badge" aria-hidden="true">{unreadCount > 99 ? '99+' : unreadCount}</span> : null}</a><a aria-label="Open profile" href="#/profile"><Avatar size="sm">{initials(profile?.displayName)}</Avatar></a></div></header>; }
...
function PostCard({ post, currentUser, echoed, liked, bookmarked, onEcho = () => {}, onQuote = () => {}, onLike = () => {}, onBookmark = () => {}, onReply = () => {}, onView = () => {}, onDelete = () => {} }) {
  ...
  const [menuOpen, setMenuOpen] = useState(false); const [replyOpen, setReplyOpen] = useState(false); const [ownerMenuOpen, setOwnerMenuOpen] = useState(false); const [confirmDelete, setConfirmDelete] = useState(false); const [deleting, setDeleting] = useState(false); const [menuPosition, setMenuPosition] = useState({ top: 0, left: 0 }); const author = post.authorHandle || post.handle || 'portal member'; const name = post.authorDisplayName || post.displayName || author; const postUrl = `${window.location.origin}/#/posts/${post.id}`; const isOwner = Boolean(currentUser?.uid && (post.authorUid === currentUser.uid || post.createdBy === currentUser.uid));
  useEffect(() => {
...
function PersonalProfile({ user, profile, posts, echoActivity, handlePurchases, echoedPostIds, likedPostIds, bookmarkedPostIds, onEcho, onQuote, onLike, onBookmark, onReply, onView, onDelete, onProfileSaved }) {
  ...
  const [tab, setTab] = useState('Posts'); const ownedPosts = posts.filter((post) => post.authorUid === user.uid || post.createdBy === user.uid); const activityPosts = tab === 'Media' ? ownedPosts.filter((post) => post.photos?.length || post.video?.url) : tab === 'Replies' ? [] : ownedPosts;
